...
{
    public class ClientesMembresiasModel : PageModel
    {
        private IClientesMembresiasPresentacion? iPresentacion = null;

        public ClientesMembresiasModel(IClientesMembresiasPresentacion iPresentacion)
        {
            try
            {
                this.iPresentacion = iPresentacion;
                Filtro = new ClientesMembresias()
                {
                    _IdClientes = new Clientes(),
                    _IdMembresias = new Membresias()
                };
            }
            catch (Exception ex)
            {
        ...
        [BindProperty] public ClientesMembresias? Actual { get; set; }
        [BindProperty] public ClientesMembresias? Filtro { get; set; }
        [BindProperty] public List<ClientesMembresias>? Lista { get; set; }
        ...
        public virtual void OnPostBtGuardar()
        {
            try
            {
                var token = HttpContext.Session.GetString("Token"); //Implementando cosas
                Accion = Enumerables.Ventanas.Editar;
                Task<ClientesMembresias>? task = null;
                if (Actual!.Id == 0)
